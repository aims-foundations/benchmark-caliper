```markdown
# Validity Extraction: HOPE: A Dataset for Dialogue-Act Classification in Counselling Conversations
<!-- Model routing: Haiku (per-page extraction) → script (registry assembly) → Sonnet (narrative) -->

## Metadata
- **Title**: HOPE: A Dataset for Dialogue-Act Classification in Counselling Conversations
- **Authors**: Ganeshan Malhotra, Abdul Waheed, Aseem Srivastava, Md Shad Akhtar, Tanmoy Chakraborty
- **Venue/Year**: Not explicitly stated in registry (inferred from content as ~2022)
- **Total Pages**: 11 (based on registry page range)
...
The benchmark's primary task is dialogue-act classification (DAC) in mental health counseling conversations, with SPARTA — a transformer-based, speaker- and time-aware architecture — proposed as the core system [Q4, Q18]. The authors deliberately distinguish counseling dialogue from both goal-oriented and chit-chat conversations [Q7, Q8, Q9], noting that counseling sessions follow a distinctive structural pattern (e.g., openings with greetings followed by problem inquiry) that makes domain-specific modeling necessary [Q10]. The classification task operates at the utterance level: given each utterance in a dyadic dialogue, the model must assign one of 12 domain-specific labels [Q22, Q58]. On the system-comparison side, the paper evaluates SPARTA against six baseline architectures spanning CNN, RNN, CRF, and transformer paradigms — CASA, SA-CRF, DRNN, ProSeqo, TextVDCNN, TextRNN, and RoBERTa [Q66, Q67, Q68, Q69, Q70, Q71, Q72] — organized into three modelling categories based on what contextual signals they incorporate [Q79, Q80]. Notably, local context (the immediate utterance neighborhood) is identified as especially important for counseling DAC [Q118], and speaker role is foregrounded as a structurally significant signal [Q125], both of which directly shape the task framing in ways relevant to downstream chatbot deployment. The paper also considers ablation variants of SPARTA [Q115] to isolate contributions of each architectural component.

### 2. Data Sources and Collection

The HOPE dataset is built from approximately 12.9K utterances drawn from 212 publicly available mental health counseling session videos sourced from YouTube [Q3, Q13, Q23, Q26]. The authors acknowledge that the primary challenge was data scarcity due to the sensitive nature of counseling content [Q23], and they emphasize that this is a "sensitive and pervasive public-health crisis" domain [Q93]. Critically for downstream validity, the paper explicitly states that "the majority of the sessions in HOPE belong to the mental health professionals and patients based in United States" [Q98], which directly constrains the cultural and demographic representativeness of the source corpus. To partially protect participant identity, synthetic names were randomly assigned to all patients and therapists [Q24]. Utterances are nearly evenly split between patients (6.38K) and therapists (6.47K) [Q54], and sessions are notably lengthy compared to other conversational datasets, with average utterance lengths of ~103 words for patients and ~124 words for therapists [Q56, Q57] — a distribution that may not reflect shorter, text-based chatbot interaction patterns in deployment. Dialogue-act label associations also vary by speaker role, with some labels primarily associated with patients and others with therapists [Q60], a distributional property with implications for how well models generalize to different conversational configurations.

### 3. Data Format and Preprocessing

Input data is text-derived from speech: transcripts were generated automatically using OTTER (an ASR tool) and subsequently hand-corrected for spelling and grammatical errors [Q25], with the authors acknowledging that "the automatic transfer of utterance from the speech modality to the text causes some information loss, though we tried our best to recover them through manual intervention" [Q94]. Patient and therapist names were systematically masked prior to release [Q97]. The dataset is split 70:20:10 for train, test, and validation [Q31, Q55, Q62], and utterance representations are extracted using a pre-trained RoBERTa model [Q61], with GRU hidden dimensions and attention blocks configured as detailed in the appendix [Q112, Q113]. The system is implemented in PyTorch via Hugging Face Transformers [Q64, Q111], and a total of ~12.9K utterances across 212 sessions constitute the full annotated corpus [Q105]. An exploratory emotion annotation pass found that ~70% of patient utterances were labeled "negative" and ~90% of therapist utterances were "neutral" [Q128], but this feature was excluded from the final model due to class imbalance [Q129]. The text-only, English-language format [Q25] matches the deployment context exactly, presenting no script or modality mismatch.

### 4. Label Categories and Output Types

The benchmark defines 12 dialogue-act labels specifically designed for counseling conversations, developed in consultation with therapists and counseling experts [Q2, Q17, Q27]. These labels are explicitly distinguished from general-purpose schemes (e.g., Switchboard) on the grounds that counseling conversations have "inherent differences with the standard conversations" [Q27]. Labels are organized hierarchically into three top-level categories — speaker initiative, speaker responsive, and general — with three labels in each of the first two and four in the general category [Q29, Q32]. The full label set is: Information Request (IRQ) [Q33], Yes/No Question (YNQ) [Q34], Clarification Request (CRQ) [Q35], Opinion Request (ORQ) [Q36], Clarification Delivery (CD) [Q37], Information Delivery (ID) [Q38], Positive Answer (PA) [Q39], Negative Answer (NA) [Q40], Opinion Delivery (OD) [Q41], Greeting (GT) [Q42], Acknowledgment (ACK) [Q43], and General Chit-Chat (GC) [Q44]. Labels are designed to capture intents of both patient and therapist and to be "easily comprehensible to assist in the development of a conversational dialogue system" [Q28]. The taxonomy is grounded in US clinical practice by construction [Q98], and while it aims for general applicability, it does not account for culturally specific communicative acts (e.g., indirect disclosure, spiritually framed coping) that may be prevalent when the system is deployed for underrepresented communities. An early experiment also explored emotion as a supplementary label dimension with three classes (positive, negative, neutral) [Q126, Q127], but this was abandoned in the final architecture [Q129]. Each utterance receives exactly one primary label [Q14, Q104], a design choice confirmed to be a simplification of counseling communication reality [Q30].

### 5. Annotation Process

Three expert linguists served as annotators [Q45], guided by a novel hierarchical annotation scheme developed specifically for the counseling domain [Q16]. Annotators were in the age range of 25–35 with 2–10 years of professional experience [Q50]. The process included a calibration phase — annotators first labeled a sample set per prepared guidelines [Q46] — followed by a group discussion of all annotations moderated by an expert therapist to ensure consistency [Q47]. Mental health professionals and linguists were consulted during guideline preparation [Q95]. While the annotators are described as linguistics experts, the paper provides no information on their cultural backgrounds, native languages, or clinical exposure to non-US or minority-community populations — an important gap given that downstream deployment targets South Asian, African American, and Latin American communities. The paper does note that "annotator bias cannot be ruled out completely" [Q96], but provides no stratified IAA analysis by label or by session type to surface where bias may be concentrated. The overall Cohen's Kappa inter-annotator agreement score of 0.7234 falls in the "substantial" category [Q48, Q49], but this aggregate score masks potential variation across labels — including those most susceptible to cultural communicative norms.

### 6. Evaluation Metrics and Output Modality

Performance is measured using macro-F1, weighted-F1, and accuracy [Q63], with SPARTA-TAA achieving state-of-the-art scores of 60.29 macro-F1, 64.53 weighted-F1, and 64.75% accuracy [Q73, Q85]. These improvements over the best baseline (CASA) are statistically significant at >95% confidence across all three metrics [Q88]. The paper also reports label-wise F1 scores [Q76], revealing that SPARTA performs well across most labels but struggles with Acknowledgment (ACK, F1=47.86%) [Q77] and that even under-represented labels like ORQ, NA, and PA achieve reasonable scores (59.09%, 64.38%, 55.84%, respectively) [Q78]. A confusion matrix analysis [Q90] identifies systematic error pairs: YNQ is confused with IRQ 26% of the time, OD with ID 43%, and ID with ACK 28% [Q91, Q92] — patterns that suggest structural ambiguity in the label ontology rather than model failure alone. Ablation results confirm that both global and local context are necessary for peak performance [Q119, Q120], that time-aware attention outperforms standard multi-head attention [Q122, Q123], and that removing speaker information degrades performance by 3–4% [Q124]. Three-fold cross-validation results are reported as consistent with the single train/val/test split [Q87]. Notably, all evaluation is framed as single-label classification accuracy; no multi-label or ranked-output metric is employed, which may constitute a systematic mismatch with the downstream chatbot use case where utterances carry multiple simultaneous communicative intents [Q30]. The optimization uses categorical cross-entropy with Adam, dropout of 0.15, and early stopping over 50 epochs [Q110].

### 7. Stated Limitations

The authors acknowledge several important limitations. Most critically for the deployment context, they explicitly state that "the majority of the sessions in HOPE belong to the mental health professionals and patients based in United States," and therefore "the effectiveness of SPARTA on data from other geographical or demographical regions may vary" [Q98, Q99]. This is a direct concession that the benchmark's validity for non-US or culturally distinct populations — including the South Asian, African American, and Latin American communities targeted by downstream chatbots — is unestablished. The single-label design is acknowledged as a simplification: "sometimes, an utterance can have multiple dialog-acts; however, they are rare in the annotated dataset. Hence, for simplicity, we consider only one (primary) label for each utterance" [Q30], which the user community has flagged as a systematic oversimplification for real counseling dialogues. Complementary label ambiguity is acknowledged structurally, as "the broad definitions of speaker-initiative and speaker-responsive dialogue-act label pairs, IRQ & ID; ORQ & OD, and CRQ & CD, seem complementary in nature" [Q51, Q52], and error analysis confirms these pairs account for some of the largest classification errors [Q91, Q92]. Annotator bias is flagged as a residual concern that "cannot be ruled out completely" [Q96]. High-stakes ethical considerations are noted, with the authors stating they "do not make any diagnostic claims" and that deployment must account for "safety-risks and mitigating any sources of bias" [Q100, Q101, Q102, Q103]. Emotion features were dropped due to severe class imbalance [Q129], and only a subset of the dataset (not the full corpus) has been publicly released [Q20]. The component-level ablation additionally shows that removing either local or global context degrades performance [Q120], confirming architectural dependencies that limit portability of results to simpler systems.

### 8. Authors and Affiliations

The paper is authored by Ganeshan Malhotra, Abdul Waheed, Aseem Srivastava, Md Shad Akhtar, and Tanmoy Chakraborty [Q11], affiliated with BITS Pilani (Goa, India), Maharaja Agrasen Institute of Technology (New Delhi, India), and IIIT-Delhi (India) [Q12]. This is an entirely India-based academic team, which provides an interesting counterpoint to the benchmark's US-clinical data source — the research team itself originates from a South Asian context, yet the dataset they built is explicitly US-clinical in character [Q98]. Funding acknowledgments reference the Ramanujan Fellowship, the ihub-Anubhuti-iiitd Foundation (under DST's NM-ICPS scheme), and the Centre for Artificial Intelligence at IIIT-Delhi [Q106], indicating Indian government and institutional research sponsorship. Computational infrastructure comprised a Tesla V100 GPU with 32GB memory running Ubuntu 18.04 [Q108], with experiments managed via PyTorch, PyTorch Lightning, Hugging Face Transformers, and WandB [Q107, Q109].
```
