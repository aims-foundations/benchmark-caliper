```markdown
# Validity Extraction: CrisisBench: Benchmarking Crisis-related Social Media Datasets for Humanitarian Information Processing
...
- **Venue/Year**: Not explicitly stated in registry (inferred from content: ~2020–2021)
- **Total Pages**: 16
- **Quotes Extracted**: 127
...
The benchmark organizes disaster-response classification into three broad task types: informativeness (binary: informative vs. not-informative), humanitarian information types (multi-class, e.g., affected individuals, infrastructure damage), and event types (e.g., flood, earthquake, fire) [Q3]. Of these, the present study implements and benchmarks only the first two — informativeness and humanitarian classification — explicitly deferring event-type classification to future work [Q22, Q23]. The humanitarian task is characterized as substantially harder than the binary informativeness task due to its multi-class nature [Q89], and event-aware training experiments suggest that disaster-specific context further shapes performance [Q93]. For the Argentine deployment context, the top-level categories (informative/not-informative; humanitarian type) are broadly relevant for triage, but the benchmark's task taxonomy does not include sub-labels for regional hazard types (pampero, ENSO drought, Río de la Plata flooding) or informal-settlement impact framing [Q61] — a gap explicitly flagged as desirable by deployment users. Experiments on individual dataset subsets (CrisisLex and CrisisNLP, with six and eleven class labels respectively) further illustrate the variability in categorical granularity across source datasets [Q76].

---

### 2. Data Sources and Collection

The benchmark consolidates eight publicly available, human-annotated tweet datasets — CrisisLexT26, CrisisLexT6, CrisisNLP, SWDM2013, ISCRAM2013, Disaster Response Data (DRD), Disasters on Social Media (DSM), CrisisMMD, and AIDR — yielding 166,100 tweets for informativeness and 141,500 for humanitarian classification [Q1, Q7, Q10]. These constituent datasets span disaster events from 2010 through 2017, providing temporal breadth [Q100, Q105], but their geographic coverage is concentrated in North American and global events (e.g., Joplin tornado, Hurricane Sandy, 2017 international disasters) [Q27, Q28, Q31], with no evident representation of Latin American or Argentine disaster contexts. English tweets dominate at 94.46% of the informativeness set [Q46], while non-English content is largely confined to the CrisisLex dataset [Q47] — meaning the benchmark has minimal Spanish-language social media content and essentially no Rioplatense Spanish informal discourse, which is a core channel for the Argentine deployment. The AIDR dataset was labeled by domain experts through the AIDR system [Q32, Q111], while other datasets were annotated via crowdsourcing or academic pipelines with varying protocols; the consolidated dataset is released under the licenses of the original sources [Q103]. Future research directions acknowledge the potential value of multilingual pre-trained models applied to the full multilingual dataset [Q98], but the current benchmark experiments are English-only [Q102].

---

### 3. Data Format and Preprocessing

The input modality is exclusively Twitter text (tweets), preprocessed through tokenization using a modified Tweet NLP tokenizer with lowercasing, URL removal, punctuation stripping, and user-ID removal [Q38]. Near-duplicate detection uses a bag-of-ngrams (uni- and bi-gram) cosine-similarity approach with a threshold of 0.75, manually validated across multiple threshold bins [Q39, Q40, Q112], and duplicate filtering reduces the dataset by approximately 25% for informativeness and 20% for humanitarian tasks [Q41]. Preprocessing also assigns a language tag to each tweet via the Google Cloud language detection API where tags were absent [Q44, Q45], addressing the challenge that some source datasets contained multilingual content — including Spanish and Italian tweets — without explicit language labeling [Q42, Q43]. Although the consolidated dataset retains multilingual content, all benchmark classification experiments use English-only tweets [Q59], and data is split 70/10/20 for train/dev/test [Q60]. For the Argentine deployment, the text-only format is well-matched [IF is LOWER priority], but the exclusive reliance on tweet-length inputs raises questions about generalization to longer-form news articles and press releases that constitute additional channels in the deployment. The dataset is released in multiple forms — mapped, duplicate-filtered, and experiment-ready subsets — to support reproducibility [Q13].

---

### 4. Label Categories and Output Types

The label ontology was constructed by manually mapping semantically similar class labels across the eight source datasets into a common schema oriented toward humanitarian aid relevance [Q18, Q33]. Labels deemed non-humanitarian — such as "animal management" and "not labeled" from CrisisNLP and CrisisLex — were excluded from consolidated experiments but retained in released data [Q34, Q57, Q58]. Specific mappings are documented (e.g., "building damaged" → "infrastructure and utilities damage") [Q33], with full mapping tables provided for each source dataset [Q106, Q107, Q108, Q109, Q110, Q123, Q124, Q125]. Critically for the deployment context, the output ontology is limited to informativeness (binary) and humanitarian type (multi-class); there is no native credibility, trustworthiness, or misinformation-likelihood label — meaning the benchmark's output space is a necessary but insufficient foundation for the Argentine use case, which requires reliability scoring [Q88]. Some humanitarian class labels carry very low support (e.g., only 17 "terrorism-related" tweets in CrisisNLP [Q52]), and class distributions remain imbalanced even after consolidation, with "not humanitarian" comprising 37.40% of the humanitarian set [Q55, Q56]. The DSM dataset contributes only a relevant/not-relevant binary label [Q109], while CrisisMMD's humanitarian labels required only minor remapping [Q110], illustrating the variation in original label granularity that the consolidation must reconcile.

---

### 5. Annotation Process

NOT FULLY DOCUMENTED: The annotation process is only minimally described in the paper. The single directly documented fact is that the AIDR dataset was labeled by domain experts using the AIDR system across different events [Q111]. For the remaining seven source datasets, the paper cites the original publications but does not reproduce annotator demographics, recruitment methods, inter-annotator agreement (IAA) statistics, or quality assurance protocols within this paper. This absence is a validity-relevant finding for the Argentine deployment: without knowledge of annotator backgrounds, it is not possible to assess how well the label judgments reflect Argentine institutional or journalistic norms for what constitutes a credible or informative disaster signal. The elicitation summary flags this annotator-population mismatch as a key gap — specifically the risk that crowdsourced or North American/European academic annotators would not share the more fluid Argentine public-discourse conventions [Q94] — and the paper provides no evidence to mitigate this concern.

---

### 6. Evaluation Metrics and Output Modality

The benchmark evaluates classifiers using weighted average precision, recall, and F1-measure, with weighting chosen explicitly to account for class imbalance [Q75]. Three model families are benchmarked: CNN (the prior state-of-the-art for disaster classification [Q63]), fastText with Common Crawl pre-trained English embeddings [Q64], and transformer models BERT, DistilBERT, and RoBERTa [Q66]. The best-performing system achieves 0.866 F1 for informativeness and 0.829 F1 for humanitarian classification on the consolidated English test set [Q84], with transformer models generally outperforming CNN and fastText — particularly on minority classes [Q90, Q91]. Notably, cross-dataset evaluation reveals substantial performance degradation: training on CrisisLex and evaluating on CrisisNLP yields a 14.3% F1 drop for informativeness [Q87], which signals that domain shift across disaster events is a real concern and raises questions about how well models trained on these datasets would generalize to Argentine disaster discourse. Event-aware training improves humanitarian classification by 1.3 F1 points using CNN [Q92], though pre-trained models were trained primarily on non-Twitter text, a limitation the authors acknowledge [Q65]. For the Argentine deployment, the benchmark's output is a discrete classification label rather than a continuous credibility or reliability score; translating these outputs into the aggregated trustworthiness signal that policy makers require is user-acknowledged as a downstream construction step, not something the benchmark itself supplies [Q2, Q96]. Hyperparameter details for all models are fully documented [Q113–Q121].

---

### 7. Stated Limitations

The authors identify several structural limitations directly relevant to validity assessment. First, the community has lacked standardized train/dev/test splits, making cross-study comparison unreliable [Q4, Q16] — a gap this paper explicitly aims to close [Q17]. Second, constituent datasets contain noisy, duplicate, and near-duplicate content [Q5, Q14, Q15], which the authors address through deduplication but at the cost of a ~25% data reduction [Q41]. Third, some datasets contain multilingual tweets without language tags [Q6], and while the authors assign language tags via Google Cloud API, the benchmark's classification experiments remain English-only [Q59], rendering the benchmark substantially inapplicable to Spanish-language Argentine social media without adaptation. Fourth, class label imbalances persist even after consolidation [Q49, Q55, Q56], and rare classes (e.g., "disease related" appears only in CrisisNLP [Q53]) limit the humanitarian classifier's reliability on underrepresented event types. Fifth, the authors explicitly note that the event-type classification task — one of the three core tasks in crisis NLP — is out of scope for this study [Q22], and that heterogeneous training data from diverse global disaster events may produce suboptimal classifiers due to domain variation [Q80]. For the Argentine deployment specifically, the combination of English-only experiments [Q59], no Argentine or Latin American event coverage, pre-trained models based on non-Twitter text [Q65], and the acknowledged instability of transformer models requiring ten re-runs per experiment [Q72] collectively suggest meaningful generalization risk when applied to Argentine crisis communication.

---

### 8. Authors and Affiliations

All four authors — Firoj Alam, Hassan Sajjad, Muhammad Imran, and Ferda Ofli — are affiliated with the Qatar Computing Research Institute (QCRI), HBKU, Qatar [Q9]. This single-institution, Middle East-based academic origin signals that the benchmark was designed and validated by researchers whose primary disaster-communication reference points and institutional context are distinct from both North American norms and Argentine crisis communication practices. While QCRI has a strong international reputation in crisis NLP, the absence of co-authors from Latin American institutions or with Argentine/Ibero-American domain expertise means that no regional calibration of label conventions, communicative norms, or hazard-type relevance was embedded in the benchmark design.
```
