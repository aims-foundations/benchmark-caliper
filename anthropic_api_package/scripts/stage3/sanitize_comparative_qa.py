...
def parse_cleaned_questions(text: str, original_questions: list[dict]) -> list[dict]:
    """Parse Opus output back into elicitation_questions.json format.

    Falls back to the original question if a Q-line can't be parsed, so that
    a partial model response never silently drops a question.
    """
    cleaned = []
    for orig in original_questions:
        qid = orig["id"]
        n = qid.lstrip("Q")
        # Match Q line up to the corresponding A line; use lookahead for A-line
        # or end-of-string for the last pair
        pattern = rf'{qid}\s*\[(\w+)\]:\s*(.+?)(?=\nA{n}:)'
...
def main() -> None:
    ...
    # === Parse and write cleaned questions ===
    cleaned_questions = parse_cleaned_questions(result, questions)
    ...
    out_path = target_dir / "elicitation_questions.json"
    out_path.write_text(json.dumps(cleaned_questions, indent=2) + "\n")
    print(f"[clean] wrote cleaned questions to {out_path}")
