...
if command -v pdftk &>/dev/null; then
    ...
    # pypdf fallback — works anywhere Python is available
    python3 - "$PDF_PATH" "$OUTPUT_DIR" <<'PYEOF'
import sys
...
for i, page in enumerate(reader.pages, start=1):
    ...
    with open(out / f"page_{i:03d}.pdf", "wb") as f:
        writer.write(f)
PYEOF
else
    echo "ERROR: No PDF splitter available. Install pdftk, qpdf, or pypdf." >&2
    exit 1
fi
