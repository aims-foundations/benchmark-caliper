...
provides deployment context within ~30 seconds, then heavy processing follows.
    ...
    Step 9              Review PDF: generate section PDFs + concatenate (local, no API)
...
def step_2_questions(
    ...
    if q_path.exists():
        print(f"[2-questions] Using cached questions: {q_path}")
        persona_prompt_path = _persona_prompt_path(name, slug) if persona_id else None
...
def step_2_collect_stdin(name: str, slug: str) -> Path:
    """Collect elicitation answers, either from a pre-filled file or interactively.

    If elicitation_answers.json already exists on disk (e.g. filled in manually
    from a template), skips interactive prompting and returns the existing path.
    Otherwise prompts each question via stdin.
    """
    a_path = _answers_path(name, slug)
    if a_path.exists():
        print(f"[2-stdin] Answers already on disk: {a_path}")
        return a_path
    q_path = _questions_path(name, slug)
...
def step_4a_template(elicitation_summary: str, name: str, slug: str) -> list[str]:
    ...
    available = set(_list_base_templates())
    valid = [fn for fn in selected if fn in available]
    if len(valid) < len(selected):
        dropped = [fn for fn in selected if fn not in available]
        print(f"[4a-template] Dropped non-existent templates: {dropped}")
    if not valid:
        fallback = random.choice(sorted(available))
        print(f"[4a-template] No valid templates remained; falling back to: {fallback}")
        valid = [fallback]
    selected = valid
...
# ===================================================================
# Step 9 — Review PDF (local, no API)
# ===================================================================
...
def step_9_review_pdf(scoring_path: Path) -> None:
    ...
    n = process_tuple(tuple_dir, styles)
    print(f"[9] Generated {n} section PDFs")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--rerun-scoring",
        action="store_true",
        help="Clear scoring, report, and review PDF so steps 7-9 re-execute "
        "with the same composed prompt. All upstream steps stay cached.",
    )
    p.add_argument(
        "--rerun-all",
        action="store_true",
        help="Clear all cached outputs for this assessment so the full "
        "pipeline re-executes from scratch. Useful for watching the "
        "complete pipeline run end-to-end.",
    )
    p.add_argument(
        "--tpm-limit",
        type=int,
...
def main() -> None:
    ...
    # expert-experiment-namespaced stems (e.g. "expert_7f59a7837eda__ltzglue").
    ...
    try:
        ...
        if args.rerun_all or args.rerun_scoring:
            import shutil
            assess_dir = _assessment_dir(name, slug)
            if args.rerun_all:
                # Clear everything except user-provided inputs
                keep = {"deployment_description.txt", "elicitation_answers.json",
                        "elicitation_questions.json"}
                for item in sorted(assess_dir.iterdir()):
                    ...
                    if item.is_dir():
                        ...
                        item.unlink()
                    print(f"[rerun-all] Cleared {item.name}")
                # Clear paper-scoped caches
                paper_dir = PAPERS / name
                for f in paper_dir.glob("*.md"):
                    f.unlink()
                    print(f"[rerun-all] Cleared papers/{name}/{f.name}")
                ...
                if cache_dir.exists():
                    ...
                    print(f"[rerun-all] Cleared page_caches/{name}/")
            else:
                for target in [assess_dir / "scoring.json", assess_dir / "report.md"]:
                    if target.exists():
                        target.unlink()
                        print(f"[rerun-scoring] Cleared {target.name}")
                pdfs_dir = assess_dir / "pdfs"
                if pdfs_dir.exists():
                    shutil.rmtree(pdfs_dir)
                    print("[rerun-scoring] Cleared pdfs/")
                # Jump directly to steps 7-9 (all upstream outputs are on disk)
                composed_path = assess_dir / "composed_prompt.md"
                if not composed_path.exists():
                    print(
                        f"ERROR: {composed_path} not found — cannot rerun scoring "
                        f"without the composed prompt. Use --rerun-all instead.",
                        file=sys.stderr,
                    )
                    sys.exit(1)
                ...
                step_9_review_pdf(results_path)
                return
...
def _run_single_step(args: argparse.Namespace, pdf_path: Path, name: str) -> None:
    ...
    if (args.rerun_scoring or args.rerun_all) and slug and args.step in ("7", "8", "9"):
        import shutil
        assess_dir = _assessment_dir(name, slug)
        for target in [assess_dir / "scoring.json", assess_dir / "report.md"]:
            if target.exists():
                target.unlink()
                print(f"[rerun] Cleared {target.name}")
        pdfs_dir = assess_dir / "pdfs"
        if pdfs_dir.exists():
            shutil.rmtree(pdfs_dir)
            print("[rerun] Cleared pdfs/")
