...
- **Authors**: Not fully enumerated in the registry (institutional authorship noted as academic and industry researchers [Q16])
- **Venue/Year**: Not explicitly stated in the registry
- **Total Pages**: 15 (based on registry page range)
...
FOLIO defines two primary tasks: a **natural language reasoning task** in which models must determine whether a conclusion is true, false, or unknown given a set of NL premises [Q15, Q63], and a **NL-to-FOL translation task** in which models must convert NL stories into their first-order logic counterparts [Q3, Q18, Q67]. The reasoning task is framed as a logical entailment problem rooted in formal deductive inference — the benchmark's central premise being that logical reasoning should be evaluated independently of other reasoning forms [Q7]. Task complexity is operationalized by reasoning depth: the modal reasoning depth in FOLIO is four, with 28.7% of examples requiring five or more inference steps, and the dataset exhibits substantially more distinct abstract syntax trees than prior datasets [Q60, Q61]. The NL-FOL translation task additionally requires discourse-level consistency across multi-premise stories, distinguishing it from prior single-sentence translation work [Q71, Q72].

For the deployment context — scoring argument quality in competitive high school debate — this task taxonomy represents a near-total category mismatch. FOLIO exclusively covers formal deductive entailment; it contains no task types corresponding to inductive reasoning, evidence credibility assessment, rhetorical effectiveness, value-laden claim evaluation, or debate-specific structural moves such as disadvantages, counterplans, or kritiks. The benchmark evaluates a narrow slice of reasoning that the deployment elicitation explicitly identifies as "the least central" dimension of argument quality [Q7, Q15]. Experiments cover fine-tuning of BERT and RoBERTa [Q84] and few-shot prompting of LLaMA, GPT-NeoX, GPT-3, GPT-3.5-Turbo, and GPT-4 [Q88], as well as specialized logical reasoning methods like Logic-LM, LINC, and DetermLR [Q91] — none of which are evaluated on argument quality dimensions the deployment requires. The benchmark does confirm that domain knowledge of FOL is necessary for good human performance [Q112], but this finding is essentially orthogonal to the coaching platform's assessment needs.

---

### 2. Data Sources and Collection

FOLIO comprises 1,430 examples (unique conclusions) paired with 487 sets of premises, developed through 980 person-hours of expert annotation effort [Q1, Q32]. Data were collected via two complementary methods designed to achieve both logical diversity and natural language variety [Q33]. The **WikiLogic** track had annotators select random Wikipedia articles as topical seeds and write entirely new stories from scratch, yielding 304 stories with 1,353 premise pairs and 753 conclusion pairs, and accounting for 74% of the total 4,351-word vocabulary [Q34, Q38, Q58, Q62]. The **HybLogic** track used a hybrid approach: algorithmically generated syllogism templates (combining 24 valid syllogism types with conjunctions, disjunctions, and implications) that human annotators then instantiated with real-world noun phrases and clauses [Q39, Q40, Q42, Q43], producing 183 stories with 1,054 premise pairs and 682 conclusion pairs [Q58].

For the deployment context, both source tracks carry notable limitations. WikiLogic stories, while topically diverse, are observed to potentially overlap with LLM pretraining corpora, as models may have seen similar logical patterns during training [Q104]. HybLogic stories, being template-derived, exhibit learnable structural patterns that fine-tuned models can exploit, as evidenced by RoBERTa-large's higher performance on HybLogic than WikiLogic [Q105]. Neither track contains content resembling competitive debate arguments — no empirical claims with cited evidence, no value-laden assertions, and no argumentative exchanges between opposing positions.

---

### 3. Data Format and Preprocessing

All FOLIO examples are text-only, with each story consisting of natural language premise sets and conclusions annotated with parallel FOL formulas [Q64, Q65]. The FOL syntax follows standard AI community conventions as established in Russell and Norvig (2010) [Q52], and annotators wrote parallel FOL sentences for both premises and conclusions [Q37]. Logical validity is enforced by requiring that NL and FOL counterparts be logically and semantically equivalent [Q68, Q69], and the dataset is structured so the inference engine always returns a determinate answer given sufficient computational resources [Q70]. The dataset is split 70%/15%/15% into train/validation/test sets (1,001/203/226 examples) by story to prevent test contamination [Q78, Q79].

Extensive preprocessing was applied to ensure linguistic naturalness: all sentences were checked with Grammarly, reviewed by annotators with English Literature backgrounds, and screened for ambiguity [Q49, Q50, Q51]. Specific NL conventions were standardized — e.g., using "either-or" for exclusive disjunction, "A or B, or both" for inclusive disjunction, and naturalistic quantifier phrasing [Q130, Q131, Q132, Q133]. FOL-specific conventions required explicit treatment of predicate properties such as transitivity and symmetry [Q56], and tense was neglected while plural verb forms were removed [Q147]. Stories involving problematic generalizations received specifiers like "all Dan knows" to add factual hedging [Q129], and stories with biased or opinionated language were removed or rewritten [Q126, Q127]. For the NL-FOL translation task, few-shot examples include all conclusions per story [Q92], and FOL format distributions across readability scores are reported [Q154]. From a deployment standpoint, the text-only, standard American English format is the one area of alignment — the coaching platform also operates on English text with no modality mismatch [Q22, Q161].

---

### 4. Label Categories and Output Types

FOLIO's output label space is a three-way categorical scheme: **True**, **False**, or **Unknown**, applied to each conclusion given its premise set [Q36, Q66]. The logical correctness of these labels is guaranteed by automatic verification through an FOL inference engine, not by human judgment of plausibility or argument quality [Q2]. The majority baseline in the test set is 38.5%, reflecting 87 true, 78 false, and 61 unknown examples [Q94]. FOL operators supported include negation, conjunction, disjunction, implication, universal and existential quantifiers, and equality [Q138], and n-place predicates are used for expressivity [Q141]. FOL is explicitly characterized as unambiguous, in contrast to NL [Q137], and as enabling exact truth-value derivation via theorem proving [Q135, Q136]. Notably, 5% of conclusions have complex syntactic structures that pose comprehension challenges even for GPT-4 [Q120].

For the deployment context, this label ontology is categorically mismatched to requirements. The platform requires separate sub-scores across logic, evidence quality, relevance, and persuasive effectiveness, plus qualitative coaching rationales and pairwise comparative rankings — none of which can be approximated by a three-way entailment label. The deployment's "correctness" standard is defined by pluralistic human judging communities (NSDA norms, TOC circuit paradigms, coaching staff rubrics), whereas FOLIO's ground truth is mechanically derived from a formal inference engine [Q2]. Temporal logic and modal logic — which would be required for reasoning about "ought" claims and counterfactuals common in debate — are explicitly excluded from FOLIO's FOL scope [Q139, Q140]. Davidsonian and neo-Davidsonian semantics, which would enable richer event-structure representation, are also excluded [Q142].

---

### 5. Annotation Process

FOLIO was manually curated by CS undergraduate and graduate students and researchers from academia and industry [Q16], selected via strict criteria: native or near-native English proficiency, formal education in FOL or semantic parsing, and strong analytical skills [Q26, Q27, Q28, Q122, Q123]. Annotators received multiple training sessions with detailed annotation guidelines before writing any stories [Q30]. The annotation pipeline involved six stages — WikiLogic collection, HybLogic collection, NL quality control, FOL quality control, NL-FOL alignment, and FOL verification — consuming 980 person-hours [Q32]. For WikiLogic stories, annotators wrote from scratch without templates based on real-world knowledge [Q35]; for HybLogic, annotators instantiated logical templates with real-world noun phrases and clauses [Q44].

Quality control was rigorous and staged: NL quality checks were performed only by NLP or computational linguistics experts, and FOL quality checks only by FOL experts [Q29, Q124, Q125]. Manual screening found that 39.2% of stories initially suffered from quality issues (bias, factual inaccuracy, stereotypes, opinionated language), all of which were rewritten under a detailed protocol [Q48]. FOL consistency was a recurring challenge — preliminary investigations found that human-written FOL formulas suffered from consistency issues, leading to the design of an explicit FOL annotation protocol [Q53, Q54, Q143, Q144, Q145, Q146]. An additional commonsense knowledge addition pass was required to insert implicit premises needed for formal inference but not written by humans naturally [Q55]. All stories and FOL annotations were reviewed by senior researchers [Q31, Q115].

For the human baseline, expert annotators (CS students familiar with FOL) achieved 95.98% accuracy on the test set, while non-expert annotators (community college or high school students unfamiliar with FOL) achieved only 61.82% — a 34.16% gap [Q107, Q108, Q109, Q110, Q111]. Both annotator groups were native English speakers [Q110]. This annotator population — technically trained FOL experts — is entirely unlike the deployment's quality-defining population of competitive debate judges, coaches, and NSDA/TOC community members, whose expertise centers on argumentation norms, not formal logic.

---

### 6. Evaluation Metrics and Output Modality

For the NL reasoning task, the primary metric is **accuracy** — the proportion of conclusions for which the model predicts the correct three-way label (True/False/Unknown) [Q80]. For the NL-FOL translation task, two metrics are used: **Syntactic Validity** (SynV), a binary score indicating whether all generated FOL formulas pass a syntactic check [Q73, Q74], and **Inference Engine Execution Accuracy** (ExcAcc), which measures whether the translated FOL, when fed into the inference engine, reproduces the correct truth values [Q75, Q76]. Experiments are run over five randomly sampled training sets with averaged results reported [Q93]. Text complexity is measured using the Dale-Chall Readability Formula [Q59].

Key empirical findings reveal substantial model weaknesses: GPT-3.5 and GPT-4 both perform substantially lower on HybLogic than WikiLogic, with results near chance on the hybrid subset [Q102]; GPT-4 achieves only 64.16% accuracy on the test set (compared to 95.98% for expert humans), leaving a 31.82% gap [Q113]; and LLMs perform significantly better on True-labeled conclusions than False or Unknown ones [Q155, Q156, Q157]. Approximately 65% of model errors on complex problems stem from faulty reasoning chain construction, 25% from within-step logical errors, and 5% from spurious commonsense shortcutting [Q118, Q119, Q121]. The inference engine used is a Stanford CS221 module, and a custom Python FOL parser was developed to bridge syntax formats [Q148, Q149, Q150, Q151, Q153]. FOL information helps GPT-4 but not GPT-3.5 [Q162, Q163].

For the deployment context, the accuracy metric over a three-way categorical label produces no information relevant to holistic argument quality scoring. The deployment requires multi-dimensional sub-scores, qualitative coaching rationales, and pairwise comparative rankings — outputs that FOLIO's evaluation framework does not produce, measure, or validate in any form.

---

### 7. Stated Limitations

The authors explicitly acknowledge that existing NL benchmarks — including FOLIO's predecessors — are inadequate for measuring complex logical reasoning [Q8, Q9, Q10]. Prior datasets like RuleTaker and LogicNLI are criticized for limited reasoning depth (at most five steps), few distinct abstract syntax trees (fewer than 50), and small vocabularies (101 and 1,077 words respectively) [Q11, Q12, Q13], and for not being written by humans with real-world knowledge [Q14]. FOLIO is positioned as addressing these gaps, but several self-identified limitations remain directly relevant to validity assessment.

The dataset explicitly avoids normatively charged content — it steers clear of identity-linked biases and stereotypes involving race, ethnicity, gender, sexuality, nationality, class, and religion [Q47]. While this is presented as a quality feature, it means FOLIO contains essentially no content of the type the deployment must handle: contested value claims, normatively ambiguous premises, and arguments where reasonable people disagree. The authors also accept certain "psychologically fundamental generalizations" (e.g., "Tigers eat other animals") that may not be factually invariant [Q128], but these are a far cry from the evidentially disputed claims central to competitive debate.

Temporal and modal logic are out of scope [Q139, Q140], and Davidsonian event semantics are excluded as unnecessary given the dataset's predicate structure [Q142]. The NL-FOL translation metric is acknowledged as insufficiently reliable, with better metrics deferred to future work [Q77]. The dataset is intentionally small and high-quality rather than large-scale [Q114], with scaling acknowledged as beyond current resources [Q116]; the authors encourage community expansion [Q117]. GPT-4's use of commonsense shortcuts to reach wrong conclusions [Q121] is flagged as a model failure mode, but is not addressed in the benchmark design. The few-shot context window constraint (8-shot maximum for text-davinci-002) is noted as a practical limitation on prompting experiments [Q95], and GPT models' inability to produce logically equivalent FOL translations despite syntactic validity (~93%) is acknowledged [Q97]. Premise ordering is confirmed to carry no significant label-predictive signal [Q160].

---

### 8. Authors and Affiliations

The registry contains only a single quote bearing on authorship and affiliation. The paper notes that FOLIO was written by "expert annotators" [Q21], and separately that annotators were "CS undergraduate and graduate students, and senior researchers" from "academia and industry" [Q16]. No specific author names, institutional affiliations, or country of origin are enumerated in any extracted quote.

NOT FULLY DOCUMENTED: The paper does not provide author names or institutional affiliations in any of the 163 extracted quotes beyond the general characterization of academic and industry contributors [Q16, Q21]. This absence limits the ability to assess whether the benchmark's design reflects cultural, national, or disciplinary perspectives that might align with or diverge from the US-based competitive debate deployment context.
```
