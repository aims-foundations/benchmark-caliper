...
- **Authors**: Not individually enumerated in the extracted quotes (see Q21 for collective attribution to the releasing team)
- **Venue/Year**: Not specified in the extracted quotes
- **Total Pages**: 15 (based on registry page range 1–15)
...
FOLIO defines its primary task as natural language reasoning with first-order logic, requiring models to determine the truth value of a conclusion given a set of premises [Q15], and frames this as an independent evaluation of logical reasoning capability [Q7]. A secondary task — NL-FOL translation — is also introduced, wherein a natural language reasoning example is translated into its FOL counterpart [Q3, Q18, Q67]. The benchmark evaluates both tasks through supervised fine-tuning on medium-sized language models [Q4] and few-shot prompting of large language models [Q5, Q19, Q23], covering models ranging from BERT and RoBERTa [Q84] to LLaMA, GPT-3, GPT-3.5-Turbo, and GPT-4 [Q88], with additional experiments using chain-of-thought, self-consistency, tree-of-thought [Q89, Q90], and logic-specific methods [Q91].

From a deployment-context perspective, FOLIO's task scope is built around first-order predicate logic — quantifiers, syllogistic structures, conjunction, disjunction, and implication [Q43] — which aligns with the second-half FOL focus of the target course. However, the benchmark does not appear to include any purely propositional task category (no quantifier-free subset is documented), leaving the course's first-half propositional content potentially uncovered. FOLIO's reasoning depth distribution — with a modal depth of four and 28.7% of examples requiring five or more reasoning steps [Q60] — and its large AST diversity [Q61] position it as a complexity-forward benchmark, which may exceed what introductory undergraduates encounter. The NL-FOL translation task [Q67, Q71, Q72] has no direct analog in the deployment scenario and is not deployment-relevant, but the reasoning task itself maps conceptually onto the course's classification objective.

### 2. Data Sources and Collection

FOLIO's 1,430 examples (organized around 487 premise sets) were assembled through two distinct collection pipelines [Q1, Q33]. The first, WikiLogic, had annotators select random Wikipedia pages as creative seeds and then write entirely original stories from scratch [Q34], resulting in 304 stories with 753 conclusion pairs and the largest share of FOLIO's 4,351-word vocabulary [Q58, Q62]. The second, HybLogic, used a hybrid method based on syllogistic templates — combining multiple valid syllogisms with conjunction, disjunction, and implication [Q40, Q42, Q43] — and had human annotators fill in real-world content for abstract entity slots [Q44], yielding 183 stories with 682 conclusion pairs [Q58]. Together the two pipelines were designed to maximize both logical diversity (AST variety) and naturalistic language breadth [Q33, Q38], at a reported cost of 980 person-hours [Q32].

A validity-relevant concern for the deployment context is that both pipelines produce expert-authored naturalistic prose, not textbook-style templates. The WikiLogic stories in particular may overlap with text patterns seen in LLM training data [Q104], which complicates interpretation of model performance. The HybLogic component's syllogistic template structure is closer in spirit to the Hurley-style course packet templates, but its instantiation with natural-language fillers still differs substantially from the syntactically controlled pedagogical register of the deployment.

### 3. Data Format and Preprocessing

Each FOLIO story is a structured NL text consisting of n premises and m conclusions, each paired with parallel FOL annotations [Q64, Q65], with the constraint that NL and FOL sentences must be logically and semantically equivalent [Q68] and that the inference engine always returns a determinate answer given sufficient resources [Q70]. The FOL syntax follows the conventions most widely used in the AI community [Q52], including the operators ¬, ∧, ∨, →, ∀, ∃, = [Q138 — from label_categories section, but bearing on format], and predicate properties such as transitivity and symmetry are encoded explicitly as additional premises [Q56]. Stories are split 70%/15%/15% into train/validation/test by story (so test stories are entirely unseen) [Q78, Q79], and the dataset excludes template-driven generation in WikiLogic in favor of free composition [Q35].

Quality-control steps directly shape the data format: all sentences were checked with Grammarly and reviewed by annotators with English Literature backgrounds [Q49, Q50]; natural language ambiguity was eliminated where possible [Q51]; disjunction is expressed via controlled phrasing conventions [Q130, Q131]; and stories containing biased or opinionated language were removed or rewritten [Q127]. Generalization statements such as "All A are B" were given epistemic qualifiers like "all Dan knows" to add factual hedging [Q129], and naturalness was enforced by preferring constructions like "Some A is B" over existential notation [Q132, Q133]. These formatting conventions are noteworthy for the deployment context: FOLIO's naturalistic phrasing choices were deliberately designed to avoid the bare, unqualified universal statements (e.g., "All A are B") that dominate the Hurley-style course packet, meaning that the surface-form register of FOLIO inputs may systematically differ from the inputs the deployed system will encounter. The readability distribution is documented [Q154], and the fine-tuning input format adds a two-layer classification head atop the encoder [Q85].

### 4. Label Categories and Output Types

FOLIO uses a three-way label schema: each conclusion is assigned one of True, False, or Unknown [Q36, Q66]. Logical correctness is guaranteed by FOL annotations that are automatically verified by an FOL inference engine [Q2], and the FOL representation is explicitly designed to remove the ambiguity inherent in natural language [Q137]. The FOL operators in scope are negation, conjunction, disjunction, implication, universal quantifier, existential quantifier, and equality [Q138]; FOL enables deriving facts from other facts and is used as direct input to the prover to obtain exact truth values [Q135, Q136]. The majority baseline on the test set is 38.5%, with 87 True, 78 False, and 61 Unknown examples [Q94] — meaning Unknown is the smallest class at roughly 27% of the test set, a base-rate imbalance relevant to the deployment's rubric.

For the deployment context, the label mapping is a **high-priority validity concern**. The course's three-way scheme distinguishes "valid" (premises entail conclusion), "invalid" (premises entail the *negation* of the conclusion), and "indeterminate" (neither entailment nor contradiction). FOLIO's "False" label, however, is verified by an FOL prover that checks whether the conclusion's negation follows from the premises [Q153], which operationally means a "False" label corresponds to the premises entailing the negation — this is closer to the course's "invalid" definition than a mere absence of entailment would be. Nevertheless, the paper does not explicitly articulate whether "False" is reserved for entailment of the negation (strict falsity) or could include cases where the conclusion simply fails to follow; this ambiguity is a residual flagged gap. The "Unknown" label, defined by the inference engine returning neither True nor False given the premises [Q70], maps conceptually onto the course's "indeterminate" (consistent with both conclusion and negation), though the course explicitly excludes undecidable formulas from this category. The n-place predicate expressivity [Q141] and the exclusion of Davidsonian semantics [Q142] further shape what logical structures can be represented. GPT-4 struggles with the 5% of conclusions that have complex syntactic structure [Q120], suggesting that label reliability is uneven across example types.

### 5. Annotation Process

FOLIO was written and reviewed entirely by expert human annotators — CS undergraduate and graduate students and senior researchers in academia and industry [Q16, Q31] — selected on the basis of native or near-native English proficiency [Q27, Q122] and formal education in first-order logic or semantic parsing [Q28, Q123]. The annotation process was divided into six stages totaling 980 person-hours [Q32], with separate expert quality checks for NL (involving NLP/computational linguistics experts) and FOL (involving FOL experts) [Q29, Q124, Q125]. Annotators received training sessions and detailed annotation guidelines [Q30], and the FOL annotation protocol was specifically designed to maximize cross-annotator consistency in how NL sentences are rendered as FOL formulas [Q54, Q143, Q144, Q145, Q146].

Human-written FOL formulas were found in preliminary investigations to suffer from consistency issues [Q53], leading to an additional round of FOL quality control that also added commonsense premises where human annotators had implicitly assumed background knowledge [Q55]. The dataset underwent a screening pass that found 39.2% of stories had at least one quality issue, all of which were rewritten according to a detailed protocol [Q48]. Non-expert annotations on the test set achieved only 61.82% accuracy versus 95.98% for expert annotators [Q111], confirming that the annotation task is genuinely difficult and that expert quality control was consequential. Tense and plural forms of verbs were abstracted away in FOL formulas [Q147], and intrinsic predicate properties (e.g., transitivity, symmetry) were added as explicit premises [Q55, Q56]. For the deployment context, the annotator population (native English speakers with formal logic training) represents a higher expertise level than the target student population, which is a feature rather than a flaw — but it means the language register in FOLIO reflects expert writing norms, not beginner-student production.

### 6. Evaluation Metrics and Output Modality

The primary metric for the NL reasoning task is accuracy [Q80], evaluated in both fine-tuned and few-shot settings [Q19, Q23, Q93]. For NL-FOL translation, two metrics are used: Syntactic Validity (SynV), a binary score indicating whether all generated FOL formulas pass a syntactic check [Q73, Q74], and Inference Engine Execution Accuracy (ExcAcc), which feeds the translated FOL into the prover and measures whether the output truth values match the gold labels [Q75, Q76]. The FOL inference engine was adopted from Stanford's CS221 course, and a custom Python parser was developed to convert human-annotated FOL into the engine's input format [Q148, Q150, Q151]; the proving pipeline involves three steps: conversion to Python code, theorem prover execution, and truth-value output [Q153].

Performance analysis reveals that GPT-3.5 and GPT-4 in few-shot settings perform markedly better on examples with 0–3 reasoning depth than on 4–7 depth examples [Q99], and that fine-tuning can generalize to longer reasoning chains when trained on sufficiently complex examples [Q100]. Expert annotators achieve 95.98% accuracy versus GPT-4's 64.16% (a 31.82-point gap) [Q111, Q113]. Confusion matrix analysis shows a systematic bias toward predicting True, with False and Unknown achieving only 61.9% accuracy under fine-tuning and 54.0% under few-shot prompting [Q155, Q156, Q157] — a finding directly relevant to the deployment, since the course's rubric treats all three categories as equally important. The Unknown/indeterminate class (61 examples in the test set [Q94]) is the smallest class; if this base rate does not match the course's problem distribution, overall accuracy will be dominated by True/False performance and may not predict indeterminate-case reliability. Readability was measured using the Dale-Chall formula [Q59], providing a text-complexity anchor for comparison with deployment inputs. Premise shuffling experiments showed less than 1% accuracy change, indicating no spurious ordering cues [Q158, Q159].

### 7. Stated Limitations

The paper itself identifies several limitations directly relevant to the deployment context. The authors acknowledge that existing benchmarks inadequately measure complex logical reasoning [Q8, Q9], and position FOLIO as a corrective — but also note that prior datasets like RuleTaker and LogicNLI had limited vocabulary and AST diversity [Q11, Q12, Q13] and lacked human-written, real-world grounding [Q14], problems FOLIO addresses. The authors explicitly exclude temporal logic and modal logic as outside the scope of FOLIO's classical FOL definition [Q139, Q140], which aligns well with the deployment's strict classical-logic-only requirement. Davidsonian semantics is also excluded [Q142], and the metric for NL-FOL translation is acknowledged as insufficiently reliable, with better metrics left to future work [Q77].

On the input content side, the authors note that HybLogic's high logical complexity may explain why GPT-4 performs near chance on those examples [Q103], and that WikiLogic examples may have been seen in LLM training data [Q104] — both of which complicate use as a deployment benchmark. The dataset deliberately avoids stereotypes and identity-linked biases [Q47], but accepts "psychologically fundamental generalizations" (e.g., "Tigers eat other animals") that are not factually invariant [Q128], which could introduce edge cases in a pedagogical context requiring strict factual accuracy. Dataset scale is acknowledged as a limitation: the authors prioritized quality over size [Q114] and lacked resources to expand the dataset further [Q116], encouraging the community to extend the annotation protocol [Q117]. The authors also note that GPT-4 sometimes uses commonsense shortcuts to arrive at wrong truth values [Q121], and that current LLMs are limited in extrapolating to long reasoning chains [Q101]. The 8-shot prompt size was constrained by the context window of text-davinci-002 [Q95], and the conversion step from human FOL notation to the inference engine's format introduces a potential reliability gap [Q152].

### 8. Authors and Affiliations

The paper credits its dataset to expert annotators who are CS undergraduate and graduate students and senior researchers in academia and industry [Q21], but the specific author names and institutional affiliations are not enumerated in any extracted quote. The collective authorship context suggests an academic research origin with connections to both university and industry settings, consistent with the use of Wikipedia as a data seed [Q34] and the adoption of a Stanford course inference engine [Q148].

NOT FULLY DOCUMENTED: Individual author names and specific institutional affiliations do not appear in the extracted quotes. This limits direct inference about the geographic or cultural origin of the benchmark's development team, though the use of English-only annotation, native-or-near-native English speaker criteria [Q27], and US-curriculum logic conventions [Q52] all point to a primarily North American or anglophone academic context that is broadly consistent with the US undergraduate deployment scenario.
```
