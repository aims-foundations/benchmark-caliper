```markdown
# Validity Extraction: MATHDIAL: A Good Teacher Knows When to Tell
...
- **Venue/Year**: Not explicitly stated in registry (inferred from content and affiliations as ~2023)
- **Total Pages**: 20
- **Quotes Extracted**: 169
...
MATHDIAL is organized around a theoretically grounded taxonomy of teacher moves applied within one-on-one tutoring dialogues on multi-step math reasoning problems [Q3]. The taxonomy comprises four primary categories — Focus, Probing, Telling, and Generic — where Focus constrains the student toward direct progress [Q38], Probing generalizes aspects of the problem to explore underlying concepts [Q39], Telling reveals parts of the solution [Q40], and Generic covers conversational elements with limited pedagogical value [Q41]. These four top-level categories decompose into finer-grained intents, as listed in Table 2 of the paper [Q42], and are grounded theoretically in Reiser's (2004) scaffolding framework distinguishing "structure" from "problematize" moves [Q36, Q37]. The benchmark is explicitly oriented toward a constructivist, Socratic pedagogy in which scaffolding is privileged: teachers most frequently deploy Focus (37% of utterances) and Telling is the rarest move [Q65, Q67], and the system is designed to teach tutors when *not* to tell [Q5, Q96]. For the Indian metro deployment context, this Socratic orientation is a critical taxonomic concern: the taxonomy systematically foregrounds open-ended scaffolding over direct correction, but Indian professional teachers operating in exam-oriented, teacher-directed pedagogical norms [per elicitation] may judge directive Telling moves as educationally appropriate rather than as failures. The conversation lifecycle is also taxonomically constrained — it ends when the student solves the problem or after 10 minutes [Q141], and unresolved conversations are positioned as useful for training intervention classifiers [Q80] — a sequencing that may not reflect the pacing of Indian classroom interactions. Secondary tasks benchmarked include tutor response generation [Q81, Q82] and interactive end-to-end tutoring simulation [Q94, Q104], giving the taxonomy both an input-side structure (what moves teachers make) and an output-side target (generating pedagogically appropriate responses).

### 2. Data Sources and Collection

MATHDIAL consists of approximately 3,000 tutoring dialogues [Q1, Q103] generated through a semi-synthetic "human-teacher plus LLM-student" paradigm adapted from the Wizard-of-Oz (WoZ) methodology [Q2, Q17]. On the student side, an LLM (InstructGPT, text-davinci-003) simulates student turns [Q32], with student confusions sampled from ChatGPT (gpt-3.5-turbo) via chain-of-thought prompting [Q29], and the most frequent incorrect solution selected for each math word problem (MWP) [Q28, Q29]. The MWPs themselves are drawn from GSM8k, a US-origin elementary school math dataset, selected because the problems are simple enough for teachers to understand quickly while remaining challenging for students [Q118]. On the teacher side, 91 expert annotators with real teaching experience were recruited [Q26] — not crowdworkers without domain expertise — distinguishing MATHDIAL from prior corpora that rely on non-expert WoZ participants [Q10, Q13, Q14]. The paper explicitly acknowledges that using an LLM to simulate student confusion is a key limitation, noting that certain student confusions may be under- or over-represented [Q106]; critically for the Indian deployment context, the LLM student simulation was not calibrated to the deferential, low-verbalization interaction style typical of Indian metro classrooms, meaning the teacher-facing dialogue instances may feel unrealistic to Indian professional evaluators [Q53, Q54]. Two alternative collection strategies — human-human dialogue and fully synthetic LLM generation — were explored and rejected as less efficient or too error-prone [Q159, Q161, Q162].

### 3. Data Format and Preprocessing

MATHDIAL's inputs are English-language text dialogues grounded in math word problems from GSM8k, with each dialogue conditioned on a student profile specifying the type of confusion and student persona [Q34]. Student turn generation uses temperature sampling (T=0.4) from InstructGPT [Q122], while student confusion oversampling uses T=0.7 with no top-k truncation over N=50 reasoning paths per MWP [Q30], and incorrect solutions are grouped by final numeric answer and the most frequent is selected [Q31]. The dialogue history, MWP, initial student confusion, and student profile together form the grounding prompt for each teacher turn [Q33, Q34]. The dataset is split 80/20 into training and test sets [Q85], with the test set further divided into "seen" (~60% of problems also in training) and "unseen" (~40%) splits to assess generalization [Q86, Q87], and the split is released publicly [Q88]. A safety filter using the Perspective API removes conversations with toxic content (less than 1%) [Q157, Q158]. Both benchmark and deployment are English text-only, so no script, modality, or language-encoding mismatch exists between the benchmark's data format and the Indian metro deployment context.

### 4. Label Categories and Output Types

The benchmark's label ontology operates at two levels: teacher move categories and post-conversation quality assessments. Teacher moves are labeled as Focus, Probing, Telling, or Generic [Q151, Q152, Q153], with the pedagogically preferred moves (Focus and Probing) explicitly designated as productive for long-term learning [Q154], while Telling is framed as appropriate primarily when a student is stuck [Q155]. Student error types are labeled along a conceptual/arithmetic axis — approximately 80% of confusions are flagged as conceptual and around 20% as arithmetic [Q62] — a distinction operationalized because arithmetic errors can be resolved with calculators while conceptual errors require scaffolding [Q61]. Post-conversation, teachers categorize student errors into six types including unit conversion error, calculation error, missing factual knowledge, and misunderstanding of question [Q140]. For human evaluation, three dimensions are rated: Coherence, Correctness, and Equitable Tutoring [Q166], where Equitable Tutoring specifically rewards responses that give the student room to explore rather than directing them to the answer [Q98] — a definition that encodes a constructivist, anti-telling orientation. Student profile labels include six confusion types derived from research on algebra misconceptions [Q124–Q130], and student confusions are further assessed for typicality relative to a sixth-grade student [Q56, Q57]. For the Indian deployment, the label ontology's privileging of scaffolding and equitable tutoring over direct correction may systematically disadvantage responses that Indian professional teachers would judge as appropriately directive — the "Equitable Tutoring" label encodes a specific pedagogical value not universally shared across teaching cultures.

### 5. Annotation Process

MATHDIAL's annotation was conducted by 91 professionally credentialed teachers recruited through Prolific, filtered for at least 500 prior submissions and 100% completion rate [Q23, Q131, Q132], with all annotators required to pass a comprehension test with 100% accuracy before contributing data [Q24, Q25]. Demographically, 71 annotators identify as female and 18 as male [Q26], with a median age of 39 years; the majority are nationals of the UK, followed by the USA, Canada, Australia, India, and Germany [Q27]. This annotator pool is highly relevant to the HIGH-priority OC dimension: while India is listed among the represented countries, it appears as a minority contributor following the UK and USA, meaning the benchmark's ground-truth judgments of "good tutoring" are predominantly shaped by Western (UK/US) pedagogical norms rather than the exam-oriented, direct-correction norms prevalent among Indian metro professional teachers. Annotators were paid above US federal minimum wage ($12/hour equivalent) [Q133], limited to five conversations per session [Q134], and underwent an initial training phase with manual quality checks [Q142, Q144]. For teacher move validation, two independent annotators re-labeled 17 conversations (102 teacher utterances), yielding inter-annotator agreement of κ=0.60 between the two independent annotators but κ=0.49 and κ=0.34 respectively when comparing either to the original teacher [Q69] — moderate agreement that the authors themselves attribute partly to the subjective boundary between Focus and Probing [Q70, Q71]. Human evaluation of model outputs used three expert annotators rating 50 responses each, with agreements of κ=0.29, κ=0.69, and κ=0.34 across the three criteria [Q100] — notably low agreement on Coherence and Equitable Tutoring, the latter being the dimension most vulnerable to cross-cultural variation. Student profiles were diversified by using student names associated with various backgrounds and pronouns [Q123], though this diversity is nominal rather than pedagogically substantive in terms of capturing Indian classroom interaction styles.

### 6. Evaluation Metrics and Output Modality

MATHDIAL employs a multi-layered evaluation framework spanning automatic metrics, dialogue-level quality measures, and interactive simulation [Q5, Q83]. For response-level automatic evaluation, the benchmark uses sacreBLEU (sBLEU) and BERTScore comparing generated to annotated teacher responses, as well as token-level F1 against the MWP as a proxy for faithfulness [Q89, Q90], and Uptake — a measure of how coherently the teacher response follows the student's prior turn [Q49, Q50, Q93]. For end-to-end interactive evaluation, two novel metrics are proposed: Success@k (percentage of conversations where the student reaches the correct answer within k turns) [Q95] and Telling@k (percentage of conversations where the teacher reveals the final answer before the student solves it independently) [Q96], together operationalizing the benchmark's core trade-off between solution effectiveness and pedagogical restraint [Q5, Q101, Q102]. Human evaluation rates Coherence, Correctness, and Equitable Tutoring on 3-point Likert or binary scales [Q97, Q167]. The Equitable Tutoring metric — defined as giving students room for reflection and exploration [Q166] — encodes a Socratic pedagogy that may be systematically misaligned with the Indian deployment context, where "providing guidance" (the user's primary criterion) may be better captured by more directive, exam-readiness-oriented responses than the benchmark rewards. MATHDIAL exhibits higher lexical diversity (bigram entropy) than CIMA and TalkMoves, similar to NCTE and TSCC [Q46, Q47], and achieves higher Uptake than other comparison datasets [Q51, Q52], suggesting dialogues are internally coherent — though coherence with Western tutoring norms does not guarantee coherence with Indian classroom expectations.

### 7. Stated Limitations

The authors acknowledge a substantial set of limitations with direct bearing on the Indian metro deployment context. Most critically for input content validity, the LLM-simulated student is acknowledged to have a "limited understanding of human learning," meaning certain student confusions may be under- or over-represented [Q106] — and as flagged in the elicitation summary, the simulation was not calibrated to the deferential, low-verbalization student communication style characteristic of Indian metro classrooms. Teachers interacting with an LLM student may have adapted their behavior to the model's response patterns rather than exhibiting naturalistic classroom behavior [Q108], and some teachers may have been demotivated upon realizing they were not interacting with real students, potentially reducing data quality [Q109]. The taxonomy itself is acknowledged as incomplete: it covers a subset of teaching moves and does not address meta-cognitive support, rapport-building [Q112], or the use of visual instructional aids like drawings [Q113] — practices that may be relevant in Indian tutoring contexts. The benchmark is further limited to the domain of math reasoning, and the authors call for domain expansion [Q111]; the exclusive use of GSM8k problems, which are US-origin elementary school word problems, means that culturally situated problem content (e.g., Indian currency, names, or contexts) is absent from the datapoints. Automatic faithfulness metrics are noted as insufficient [Q91, Q92], and measuring only immediate solving success fails to capture long-term learning outcomes [Q114] — a limitation that may disproportionately affect validity assessments in Indian educational contexts where exam performance over time is the dominant success criterion. Prior datasets suffered from hallucinations in ground truth [Q14] and low tutoring quality [Q15], problems MATHDIAL aimed to address, though the LLM student simulation itself is noted to be error-prone to arithmetic mistakes [Q59, Q169].

### 8. Authors and Affiliations

The authorship team is drawn from three European/Swiss institutions and one Singaporean institution: ETH Zurich (ETH AI Center and Department of Computer Science), TU Darmstadt (UKP Lab and Hessian Center for AI), and Nanyang Technological University / National Institute of Education, Singapore [Q7, Q8]. Funding was provided by the ETH AI Center, the German Federal Ministry of Education and Research, the Swiss National Science Foundation, and private Swiss foundations [Q115]. No Indian institutional affiliation is represented among the authors or funders. The Nanyang Technological University connection introduces some Asian educational context via Manu Kapur's learning sciences affiliation, though NIE/NTU's pedagogical norms may differ from those of Indian NCERT/CBSE professional teachers. This Western-and-Singaporean institutional provenance is consistent with the benchmark's constructivist, Socratic pedagogical orientation and reinforces the OC-dimension concern that the benchmark's ground-truth quality judgments may not reflect the standards of credentialed Indian metro educators.
```
